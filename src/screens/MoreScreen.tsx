...
export function MoreScreen({ onOpenPOPortal }: { onOpenPOPortal: () => void }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScreenScroll maxWidth={700}>
        ...
        {user.hasPOPortal ? (
          <Pressable onPress={onOpenPOPortal}>
        ...
        <Card style={{ padding: 0, marginBottom: 14 }}>
          {[
            { icon: 'language', label: 'ভাষা', sub: 'বাংলা · English' },
            { icon: 'cloud-upload-outline', label: 'ব্যাকআপ', sub: 'গতকাল সিঙ্ক হয়েছে' },
            { icon: 'shield-checkmark-outline', label: 'নিরাপত্তা', sub: 'PIN ও বায়োমেট্রিক' },
            { icon: 'help-circle-outline', label: 'সাহায্য', sub: 'সাথীকে জিজ্ঞেস করুন' },
          ].map((it, i, arr) => (
            <Row
              key={it.label}
              gap={12}
